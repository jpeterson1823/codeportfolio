...
// Added after the fact due to errors
#include <stdlib.h>
...
void draw(void)
{
    int gridSize = sizeof(grid[0])/sizeof(grid[0][0]);
...
bool lookup(string s)
{
    ...
    while (!exit){
        if (strcmp(s,((string)dictionary.words[middle].letters)) > 0)
        {
            min = middle + 1;
            middle = (max - 1) / 2;

        }
        else if (strcmp(s,((string)dictionary.words[middle].letters)) < 0)
        {
            max = middle - 1;
            middle = max / 2;
        }
...
// Scrambles the grid by rotating it 90 degrees clockwise, whereby grid[0][0] rotates to grid[0][DIMENSION - 1]
